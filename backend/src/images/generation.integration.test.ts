...
test('video generation stores a playable MP4 with row-major reveal settings', async () => {
  const owner = await createUser('video');
  const sourceBuffer = await sharp({
    create: { width: 80, height: 40, channels: 3, background: { r: 40, g: 120, b: 230 } },
  })
    .png()
    .toBuffer();
  const sourceId = await upload(owner.accessToken, sourceBuffer, 'video-source.png');
  cleanupObjects.push(await storedObject(sourceId));

  const response = await request(app)
    .post(`/api/v1/images/${sourceId}/generate-video`)
    .set('Authorization', `Bearer ${owner.accessToken}`)
    .send({
      columns: 20,
      characterSet: '@. ',
  ...
  assert.equal(response.body.image.kind, 'GENERATED');
  assert.equal(response.body.image.mimeType, 'video/mp4');
  assert.equal(response.body.image.parentImageId, sourceId);
  assert.equal(response.body.image.settings.output, 'video');
  assert.equal(response.body.image.settings.animation, 'row-major-reveal');
  assert.equal(response.body.image.settings.durationSeconds, 2);

  const generatedId = response.body.image.id as string;
  const generatedObject = await storedObject(generatedId);
  cleanupObjects.push(generatedObject);
  assert.match(
    generatedObject.objectKey,
    new RegExp(`^users/${owner.userId}/generated/${generatedId}/animated-text\\.mp4$`),
  );
  const head = await objectStorage.send(
    new HeadObjectCommand({ Bucket: generatedObject.bucket, Key: generatedObject.objectKey }),
  );
  assert.equal(head.ContentType, 'video/mp4');
  const video = await getImageObject(generatedObject.bucket, generatedObject.objectKey);
  assert.ok(video.length > 1_000);
  ...
  const probe = spawnSync(
    ...
    { input: video, maxBuffer: 2 * 1024 * 1024 },
  );
  assert.equal(probe.status, 0, probe.stderr.toString());
  const stream = JSON.parse(probe.stdout.toString()).streams?.[0] as {
    codec_name: string;
    width: number;
    height: number;
  ...
  assert.equal(stream.height, 80);
  const lastFrame = Number(stream.nb_read_frames) - 1;
  assert.ok(lastFrame > 1);
  ...
  const frameBytes = stream.width * stream.height * 3;
  assert.equal(decoded.stdout.length, frameBytes * 2);
  let changedBytes = 0;
  for (let offset = 0; offset < frameBytes; offset += 1) {
    if (Math.abs((decoded.stdout[offset] ?? 0) - (decoded.stdout[offset + frameBytes] ?? 0)) > 8) {
      changedBytes += 1;
    }
  }
  assert.ok(changedBytes > 100, 'The final frame must reveal visible glyph pixels.');
